...
export function BottomSheet({ open, onOpenChange, title, children }: BottomSheetProps) {
  return (
    <Drawer.Root open={open} onOpenChange={onOpenChange}>
      <Drawer.Portal>
        <Drawer.Overlay className="fixed inset-0 bg-bg/80 backdrop-blur-sm z-40" />
        <Drawer.Content className="fixed bottom-0 left-0 right-0 max-w-[480px] mx-auto bg-surface border-t-2 border-gold/20 rounded-t-[24px] z-50 flex flex-col max-h-[85vh]">
          <div className="flex-shrink-0 mx-auto w-12 h-1.5 bg-border rounded-full mt-4 mb-4" />
          {title && (
